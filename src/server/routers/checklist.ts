...
export const checklistRouter = router({
  ...
  getChecklistById: procedure.input(z.string()).query(async (opts) => {
    const checklist = await prisma.checklist.findUnique({ where: { id: opts.input } })

    return checklist
  }),
  patchChecklist: procedure.input(z.object({
    id: z.string(),
  ...
  addTaskToChecklist: procedure.input(z.object({
    ...
    await prisma.checklist.update({ where: { id: opts.input.checklistId }, data: { taskOrder: { push: newTask.id } } })
  ...
  deleteTaskById: procedure.input(z.object({
    ...
    const checklist = await prisma.checklist.findUnique({ where: { id: opts.input.checklistId }, include: { tasks: true, } });
    ...
    await prisma.checklist.update({
      where: { id: opts.input.checklistId }, data: {
        taskOrder: {
  ...
  patchTask: procedure.input(z.object({
    id: z.string(),
